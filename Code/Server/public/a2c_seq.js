	...
	async function main(offline=false,use_gyro=false) {
		...
		// while(true){
			...
			if (_this.a2c.active==true){
					...
					if (epoch % 200 == 0){
						await agent.train_model(state_scaled, action, reward, next_state_scaled, true,chart);
						await environment.reset()
						...
						// if (batch % 20 == 0){
						await agent.train_model(state_scaled, action, reward, next_state_scaled, false,chart);
						// }
					}
